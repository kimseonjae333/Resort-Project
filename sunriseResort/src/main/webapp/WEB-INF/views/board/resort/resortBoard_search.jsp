...
<script language="JavaScript">
	function checkSearch() {
		...
		if (searchKeyword.length > 20) {
			...
			form.action = '/sunriseResort/resortBoard_searchNext?search=${boardItem.title}';
			form.submit();
		}
	}
</script>
			...
			<table cellspacing="0" width="800" border="1" align="center">
				...
				<c:forEach var="boardItem" items="${pages.content}">
					<tr>
						...
						<td><p class="p">
								<a href="/sunriseResort/resortBoard_view?id=${boardItem.id}">
									<c:choose>
				...
				<div class="pagination">
					<div class="center">
						<c:if test="${pages.number > 0}">
							<a href="resortBoard_searchNext?search=${search}&page=${pages.number - 1}">&lt;</a>
						</c:if>

						<c:forEach items="${pageNumbers}" var="i">
							<c:choose>
								<c:when test="${i eq pages.number}">
									<a href="resortBoard_searchNext?search=${search}&page=${i}" style="color: red">${i + 1}</a>
								</c:when>
								<c:otherwise>
									<a href="resortBoard_searchNext?search=${search}&page=${i}" style="color: black">${i + 1}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
					...
					<table width="800">
						<tr>
							<td style="float: right;"><input type="text" name="search"
								...
								onclick="location.href='/sunriseResort/resortBoard_list'"></td>
						</tr>
					</table>
				</form>
			</div>
